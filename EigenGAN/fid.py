# from https://github.com/rosinality/stylegan2-pytorch/blob/master/fid.py
...
def extract_loader_features(loader, inception, device):
    ...
    for img in tqdm(loader, desc="extracting ref features for FID: ", leave=False,disable=True):
        img = img.to(device)
...
@torch.no_grad()
def extract_model_features(model, inception, device, num_samples=50000, batch_size=64):

    num_iters = int(np.ceil(num_samples / batch_size))

    feature_list = []
    for _ in tqdm(range(num_iters), desc="extracting generated data features for FID: ", leave=False,disable=True):
...
def fid_generator(generator, ref_mean, ref_cov, inception, device, num_samples, batch_size=64):
