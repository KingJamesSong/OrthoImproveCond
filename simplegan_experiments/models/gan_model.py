...
import numpy as np
import itertools
from .base_networks import ConvTranspose2dBlock, Conv2dBlock
...
class GANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if opt.isTrain:
            ...
            if opt.reg_type == 'olr':
                print(self.netG)
                my_list1 = ['layers.0.layers.conv']
                params1 = list(map(lambda x: x[1], list(filter(lambda kv: kv[0] in my_list1, self.netG.named_parameters()))))
                base_params = list(map(lambda x: x[1], list(filter(lambda kv: kv[0] not in (my_list1),self.netG.named_parameters()))))
                self.optimizer_G = torch.optim.Adam([{'params': params1}, {'params': base_params}], lr=opt.lr,
    ...
    def optimize_parameters(self):
        ...
        self.backward_G()
        if self.reg_type == 'nog':
            self.orthogonal_gradient()
        if self.reg_type == 'olr':
            weight_grad = self.netG.layers[0].layers[0].weight.grad.view(-1, 1)
            weight = self.netG.layers[0].layers[0].weight.view(-1,1)
            wtw=weight.t().mm(weight)
            ltw=weight_grad.t().mm(weight)
            ltl=weight_grad.t().mm(weight_grad)
            lr = float((wtw*ltw)/(wtw*ltl+2*ltw*ltw))
            if lr>0 and lr < self.optimizer_G.param_groups[-1]['lr']:
                self.optimizer_G.param_groups[0]['lr'] = lr
            else:
                self.optimizer_G.param_groups[0]['lr'] = self.optimizer_G.param_groups[-1]['lr']
        self.optimizer_G.step()
        if self.reg_type == 'vp':
            self.optimizer_C.step()

    def orthogonal_gradient(self):
        weight_grad = self.netG.layers[0].layers[0].weight.grad
        B, C, H, W = weight_grad.size()
        u, s, vh = torch.linalg.svd(weight_grad.view(B, C * H * W),full_matrices=False)
        s[:] = 1
        self.netG.layers[0].layers[0].weight.grad = u.mm(torch.diag(s)).mm(vh).view(B, C, H, W)
